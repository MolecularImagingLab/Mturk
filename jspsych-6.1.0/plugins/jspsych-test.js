...
jsPsych.plugins['test'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    const n  = trial.questions[0].labels.length;     // Number of item responses
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      var question = trial.questions[item_order[i]];
      // Define item ID.
      const qid = ("0" + `${item_order[i]+1}`).slice(-2);

      // Define response values.
      var values = [];
      for (var j = 0; j < question.labels.length; j++){ values.push(j); }
      ...
      if (i % trial.scale_repeat == 0) {
        ...
        for (var j = 0; j < questions.labels.length; j++) {
          html += `<div class="survey-template-header">${question.labels.labels[j]}</div>`;
        }
      }

      // Add row.
      html += '<div class="survey-template-row">';
      html += `<div class='survey-template-prompt'>${question.prompt}</div>`;
      for (let v of values) {
