...
jsPsych.plugins['survey-template-obj'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'survey-template-obj',
    description: '',
    parameters: {
      items: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // mod added scale[0]
    var propNames = Object.getOwnPropertyNames(trial.scale)
    const n  = trial.scale[propNames[0]].length;   // Number of item responses, assuming here that all items have the same number
    ...
    for (var i = 0; i < trial.items.length; i++) {
      ...
      // mod added scale[i]
      var values = [];
      for (var j = 0; j < trial.scale[propNames[propCount]].length; j++){ values.push(j); }
      ...
      if (i % trial.scale_repeat == 0) {
        ...
        for (var j = 0; j < trial.scale[propNames[propCount]].length; j++) {
          html += `<div class="survey-template-header">${trial.scale[propNames[propCount]][j]}</div>`;
        }
        if (propCount < propNames.length - 1) {
        propCount +=1
        } else {
          propCount = 0
        };
